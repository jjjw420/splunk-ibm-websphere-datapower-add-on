...
sys.path.append(os.path.join(splunkhome, 'etc', 'apps', 'TA-dp', 'lib'))
...
@Configuration()
class DPConfigCommand(ReportingCommand):
    ...
    """
    follow_dependencies = Option(
        doc='''
        **Syntax:** **follow_dependencies=***<follow_dependencies>*
        **Description:** Follow object dependencies. Default: false''',
        require=False, default=False, validate=validators.Boolean())
    only_show_top_levels = Option(
        doc='''
        **Syntax:** **only_show_top_levels=***<only_show_top_levels>*
        **Description:** Only show the objects at the very top of the tree(eg. no more parents). Default: false''',
        require=False, default=False, validate=validators.Boolean())
    output_dependency_tree = Option(
        doc='''
        **Syntax:** **output_dependency_tree=***<output_dependency_tree>*
        **Description:** Output the dependency tree. Default: false''',
        require=False, default=False, validate=validators.Boolean())     
    print_text_dependency_tree = Option(
        doc='''
        **Syntax:** **print_text_dependency_tree=***<print_text_dependency_tree>*
        **Description:** Print the tree as text. Default: false''',
        require=False, default=False, validate=validators.Boolean())
    print_json_dependency_tree = Option(
        doc='''
        **Syntax:** **print_json_dependency_tree=***<print_json_dependency_tree>*
        **Description:** Print the tree as json. Default: false''',
        require=False, default=False, validate=validators.Boolean())        
    ...
    #     self.logger.error('IN MAP!!')
    #     follow_dependencies = self.follow_dependencies
    #     only_show_top_levels = self.only_show_top_levels
    #     output_dependency_tree = self.output_dependency_tree
    #     print_text_dependency_tree = self.print_text_dependency_tree
    #     print_json_dependency_tree = self.print_json_dependency_tree

    #     self.logger.error('follow_dependencies: %s', str(follow_dependencies))
    #     self.logger.error('only_show_top_levels: %s', str(only_show_top_levels))
    #     self.logger.error('output_dependency_tree: %s', str(output_dependency_tree))
    #     self.logger.error('print_text_dependency_tree: %s', str(print_text_dependency_tree))
    #     self.logger.error('print_json_dependency_tree: %s', str(print_json_dependency_tree))
        
    #     query = "search index=_* earliest=-5m | stats count"

    #     self.logger.error('records type: %s', str(type(records)))
    #     self.logger.error('records fieldnames: %s', str((records.fieldnames)))
    #     obj_device = None
    #     obj_domain = None
    #     obj_type = None
    #     obj_name = None
    #     self.logger.error('IN MAP b4 loop!!')
    #     for record in records:
    #         #yield { "device": obj_device, "domain": obj_domain, "type": obj_type, "name": obj_name}
    #         yield self.recurse_up(record)

    #     end_time = time.time()

    def reduce(self, records):
        self.logger.error('IN REDUCE!!:')
        start_time = time.time()
        follow_dependencies = self.follow_dependencies
        only_show_top_levels = self.only_show_top_levels
        output_dependency_tree = self.output_dependency_tree
        ...
        new_recs = []
        self.logger.error('records type: %s' % str(type(records)))
        def recurse_up(in_obj):
            
            self.logger.error('recurse up: %s' % str(in_obj))
            obj_device = None
            obj_domain = None
            obj_type = None
            obj_name = None
            if "domain" in in_obj:
                if isinstance(in_obj["domain"], list):
                    obj_domain = in_obj["domain"][0] 
                else:
                    obj_domain = in_obj["domain"]    

            if "device" in in_obj:
                if isinstance(in_obj["device"], list):
                    obj_device = in_obj["device"][0] 
                else:
                    obj_device = in_obj["device"]    

            if "type" in in_obj:
                if isinstance(in_obj["type"], list):
                    obj_type = in_obj["type"][0]
                else:    
                    obj_type = in_obj["type"]

            if "name" in in_obj:
                if isinstance(in_obj["name"], list):
                    obj_name = in_obj["name"][0]
                else:    
                    obj_name = in_obj["name"]
            else:
                if "_raw" in in_obj:
                    in_obj_o = json.loads(in_obj["_raw"])
                    if "name" in in_obj_o:
                        obj_name = in_obj_o["name"]

            self.logger.error('recurse up 2: %s' % str(in_obj))
            self.logger.error('recurse up 2 obj_type: %s' % str(obj_type))
            self.logger.error('recurse up 2 obj_name: %s' % str(obj_name))
            
            h_ref = "/mgmt/config/{domain}/{type}/{name}"
            if obj_type is not None and obj_name is not None:
                h_ref = "/mgmt/config/{domain}/{type}/{name}".format(domain=obj_domain, type=obj_type, name=obj_name)

            c_query = """
                search index={index_name} sourcetype="_json" earliest=0 device="{device}" domain="{domain}" "{h_ref}" type!="DomainAvailability" 
            """.format(index_name=index_name, domain=obj_domain, device=obj_device, h_ref=h_ref)
            self.logger.error('recurse up 2 c_query: %s' % str(c_query))
            kwargs_oneshot = { "count": 0 }
            response = self.service.jobs.oneshot(c_query, **kwargs_oneshot)
            res = splunklib.results.ResultsReader(response)
            #self.logger.error('res: %s', str(res))
            for r in res:
                #self.logger.error('r: %s', str(r))
                recurse_up(r)

            #self.logger.error('before return : %s' % str({"_time": in_obj["_time"], "device": obj_device, "domain": obj_domain, "type": obj_type, "name": obj_name}))
            self.logger.error('len new_recs: %s' % str(len(new_recs)))
            if {"device": obj_device, "domain": obj_domain, "type": obj_type, "name": obj_name} not in new_recs:
                new_recs.append({"device": obj_device, "domain": obj_domain, "type": obj_type, "name": obj_name})

        self.logger.error('b4 loop')
        if records is not None:
            self.logger.error('records not none')
            for record in records:
                #self.logger.error('record type: %s' % str(type(record)))
                #self.logger.error('record: %s' % str(record))
                #yield record
                self.logger.error('b4 recurse:')
                r = recurse_up(record)    
                self.logger.error('after recurse')
                #yield record 
            self.logger.error('after for')
        self.logger.error('new_recs %s' % str(new_recs))    
        for r in new_recs:
            r["_time"] = time.time()
            yield r
        self.logger.error('adfter loop')
        end_time = time.time()

    # def recurse_up(self, in_obj):
    ...
    #     self.logger.error('before return : %s' % str({"_time": in_obj["_time"], "device": obj_device, "domain": obj_domain, "type": obj_type, "name": obj_name}))
    #     yield {"_time": in_obj["_time"], "device": obj_device, "domain": obj_domain, "type": obj_type, "name": obj_name}    

dispatch(DPConfigCommand, sys.argv, sys.stdin, sys.stdout, __name__)
